-- Complete database setup script for XAMPP
-- Run this single file to create everything at once

-- Create database
CREATE DATABASE IF NOT EXISTS sistem_absensi CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- Use the database
USE sistem_absensi;

-- Drop tables if exist (for clean setup) - drop in reverse order to handle foreign key constraints
...
CREATE TABLE `qr_codes` (
  ...
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_qr_code_id` (`qr_code_id`),
  KEY `idx_nidn` (`nidn`),
